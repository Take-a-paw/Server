...
class RecordWalkDetailService:
    ...
    def get_walk_detail(
        ...
        if authorization is None:
            return error_response(401, "WALK_DETAIL_401_1", "Authorization 헤더가 필요합니다.", path)

        if not authorization.startswith("Bearer "):
            return error_response(401, "WALK_DETAIL_401_2", "Authorization 헤더는 'Bearer <token>' 형식이어야 합니다.", path)

        parts = authorization.split(" ")
        if len(parts) != 2:
            return error_response(401, "WALK_DETAIL_401_2", "Authorization 헤더 형식이 잘못되었습니다.", path)

        id_token = parts[1]
        decoded = verify_firebase_token(id_token)
        if decoded is None:
            return error_response(401, "WALK_DETAIL_401_2", "유효하지 않거나 만료된 Firebase ID Token입니다. 다시 로그인해주세요.", path)
        ...
        if include_points is not None:
            if include_points.lower() in ["true", "1", "yes"]:
                ...
                return error_response(400, "WALK_DETAIL_400_1", "include_points 파라미터 값이 올바르지 않습니다.", path)
        ...
        if not user:
            return error_response(404, "WALK_DETAIL_404_1", "해당 사용자를 찾을 수 없습니다.", path)

        # 4) 산책 조회
        try:
            walk = self.repo.get_walk(walk_id)
            if not walk:
                return error_response(404, "WALK_DETAIL_404_2", "요청하신 산책 세션을 찾을 수 없습니다.", path)
        except Exception as e:
            print("WALK_DETAIL_QUERY_ERROR:", e)
            return error_response(500, "WALK_DETAIL_500_1", "산책 상세 정보를 조회하는 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.", path)
        ...
        if not family_member:
            return error_response(403, "WALK_DETAIL_403_1", "해당 산책 기록을 조회할 권한이 없습니다.", path)
